...
function parsePokemonLocations(doc, gameKey) {
  ...
  for (const tr of locTable.querySelectorAll('tr')) {
    ...
    for (const a of td.querySelectorAll('a[href^="/location/"]')) {
      ...
      let locName = a.textContent.trim();
      // PokémonDB abbreviates: "Route 1, 2, 3" — subsequent links have just the number.
      // Derive the full name from the slug (e.g. "kalos-route-3" → "Route 3").
      if (/^\d+$/.test(locName)) {
        const parts = slug.split('-');
        // Find the type word (route, cave, etc.) from the slug
        const regionless = parts.slice(1).join('-'); // drop region prefix
        locName = regionless.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      }
      locs.push({ name: locName, slug });
    }
